...
public class SurveyController {
	...
	public String list(@PathVariable String dept, ModelMap models) {
		...
		models.put("unpublishedSurveys", surveyDao.getUnpublishedSurveys(department));
		return "survey/list";
	}
	...
	public String search(@PathVariable String dept, @RequestParam String text, HttpSession session) {
		Department department = departmentDao.getDepartment(dept);
	...
	public String view(@RequestParam Long id, @RequestParam(required = false) Integer sectionIndex, ModelMap models) {
	...
	public String editQuestions(@RequestParam Long surveyId, @RequestParam(required = false) Integer sectionIndex,
			ModelMap models) {
	...
	public String clone(@PathVariable String dept, @RequestParam Long id) {
		...
		logger.info(SecurityUtils.getUser().getUsername() + " cloned survey " + newSurvey.getId() + " from "
				+ oldSurvey.getId());
	...
	public String deleteSection(@RequestParam Long surveyId, @RequestParam int sectionIndex) {
		Survey survey = surveyDao.getSurvey(surveyId);
		if (!survey.isPublished()) {
			survey.getQuestionSheet().getSections().remove(sectionIndex);
			surveyDao.saveSurvey(survey);

			logger.info(SecurityUtils.getUser().getUsername() + " deleted section " + sectionIndex + " from survey "
					+ surveyId);
		}
	...
	public String reorderQuestion(@RequestParam Long surveyId, @RequestParam int sectionIndex,
			@RequestParam Long questionId, @RequestParam int newIndex, HttpServletResponse response)
			throws IOException {
		Survey survey = surveyDao.getSurvey(surveyId);
		if (!survey.isPublished()) {
			QuestionSection questionSection = survey.getQuestionSheet().getSections().get(sectionIndex);
			...
			logger.info(SecurityUtils.getUser().getUsername() + " reordered question " + questionId + " in survey "
	...
	public String deleteQuestion(@RequestParam Long surveyId, @RequestParam int sectionIndex,
			@RequestParam Long questionId) {
		Survey survey = surveyDao.getSurvey(surveyId);
		if (!survey.isPublished()) {
			survey.getQuestionSheet().getSections().get(sectionIndex).removeQuestion(questionId);
			surveyDao.saveSurvey(survey);

			logger.info(SecurityUtils.getUser().getUsername() + " deleted question " + questionId + " in survey "
					+ surveyId);
		}
	...
	public String remove(@RequestParam Long id) {
		...
		logger.info(SecurityUtils.getUser().getUsername() + " removed survey " + id);

		return "redirect:list";
	}

	@RequestMapping("/department/{dept}/survey/publish")
	public String publish(@RequestParam Long id, HttpServletResponse response) throws IOException {
		Survey survey = surveyDao.getSurvey(id);
		if (!survey.isPublished()) {
			survey.setPublishDate(Calendar.getInstance());
			survey = surveyDao.saveSurvey(survey);
			logger.info(SecurityUtils.getUser().getUsername() + " published survey " + id);
		}

		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
		response.setContentType("text/plain");
		response.getWriter().print(dateFormat.format(survey.getPublishDate().getTime()));

		return null;
	}

	@RequestMapping("/department/{dept}/survey/close")
	public String close(@RequestParam Long id, HttpServletResponse response) throws IOException {
		Survey survey = surveyDao.getSurvey(id);
		if (!survey.isClosed()) {
			survey.setCloseDate(Calendar.getInstance());
			survey = surveyDao.saveSurvey(survey);
			logger.info(SecurityUtils.getUser().getUsername() + " closed survey " + id);
		}

		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
		response.setContentType("text/plain");
		response.getWriter().print(dateFormat.format(survey.getCloseDate().getTime()));
	...
	/* API related survey methods */
	@RequestMapping("/api/department/survey/list")
	public void list(HttpServletRequest request, HttpServletResponse response, @RequestParam(name="dept") String dept) {
		Department department = departmentDao.getDepartment(dept);
		...
		for (Survey survey : openSurveys) {
			jsonArray.add(gson.toJson(survey));
		}
